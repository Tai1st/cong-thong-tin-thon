...
Mục tiêu: xây một hệ thống **SaaS multi-tenant** — nhiều thôn/xã dùng chung 1 bộ mã nguồn (FE + BE), mỗi thôn là 1 tenant độc lập, truy cập qua **subdomain riêng** (vd `doan-ket.tenxa.vn`, `yen-khanh.tenxa.vn`), dữ liệu tách biệt hoàn toàn theo tenant. Thôn "Đoàn Kết" hiện tại trở thành tenant thật đầu tiên.
...
| Backend | NestJS + TypeScript | Repo riêng `cong-thong-tin-thon-be`, expose REST API |
...
sequenceDiagram
    ...
    Browser->>Nginx: GET https://doan-ket.tenxa.vn/dashboard-admin
    Nginx->>NextMiddleware: proxy (giữ Host header)
    NextMiddleware->>NextMiddleware: tách subdomain "doan-ket" từ Host
    NextMiddleware->>FEServer: forward request + header x-tenant-slug=doan-ket
...
- **Dev local:** dùng `*.localhost` (vd `doan-ket.localhost:3000`) — trình duyệt/Node hiện đại tự resolve về `127.0.0.1`, không cần sửa hosts file.
...
- Bản đồ dùng **Leaflet + tile OpenStreetMap** (thay vì SVG vẽ tay như bản mẫu) — mỗi thôn hiển thị bằng 1 marker/polygon tại tọa độ thật (`Tenant.lat`/`Tenant.lng`, thêm mới vào schema `Tenant` ở mục 6), thay vì path SVG cố định.
- Dữ liệu render: `GET /tenants/public` (BE, không cần auth) trả `{ slug, name, lat, lng }` cho mọi tenant đang active — thay thế mảng `REGIONS` cứng trong bản HTML mẫu.
...
erDiagram
    ...
    TENANT {
        ...
        string name
        number lat "vị trí trên bản đồ danh mục (mục 4.1)"
        number lng
        date createdAt
...
- **GPS hộ gia đình** (`Household.gpsCoord`, lấy từ Geolocation API trình duyệt khi cư dân định vị nhà mình): hiển thị trực quan bằng **Leaflet + tile OpenStreetMap** trên dashboard Cư dân (xem vị trí nhà mình) và Trưởng thôn (xem toàn bộ hộ trên 1 bản đồ) — cùng thư viện dùng ở trang danh mục tenant (mục 4.1), không thêm lib map thứ hai.
...
1. **Khởi tạo BE (`cong-thong-tin-thon-be`)**: NestJS project, module `tenant` + `auth` + schema Mongoose theo mục 6, script migrate dữ liệu mẫu từ `js/data.js` (`E:\Dev\cong-thong-tin-thon`, thôn "Đoàn Kết") thành tenant thật đầu tiên.
2. **Khởi tạo FE (`cong-thong-tin-thon-fe`)**: Next.js project mới, dựng lại UI theo `E:\Dev\cong-thong-tin-thon`, thêm `middleware.ts` tách subdomain, gọi API BE thật + cookie JWT (không dùng localStorage).
